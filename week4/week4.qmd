...
*Response:* reef_slope (Wall) 2% vs 7.5%, Adam said walls have a big impact on fish mass and reef_type (Patch) and High pollution_risk have 0 which almost defeats the point of a control?
...
*Response:* reef_slope wall definitely will confound the estimate, Adam mentioned how steep slopes drastically increase fish biomass due to upwelling and nutrient influx and to only have 1 control for a wall slope vs 8 for the MPA slope does not seem apples-to-apples
...
*Response:* How would fish biomass change (be affected) at an MPA site if the same exact site was not an MPA site.
...
*Response:* 108 matched, 37 control included

-   ESS = Effective Sample Size

If were using the same control repeated it must be a pretty good control so wouldn't that increase our statistical confidence? Or is it because we get less spread of possible control values.

**Q6.** Find the covariate with the largest standardized mean difference (SMD) *after matching*. Which covariate is it?

*Response:* Distance to deep water was nearly a difference of \~1.3
...
*Response:* On average MPA sites have 370 kg/ha more ecological biomass than non-MPA sites.

**Q8.** What is the biggest remaining threat to causal interpretation of regression coefficients after matching?

*Response:* We have found some observed confounding variables
...
```{r}
fe_lm <- mod1_nofe <- lm(perinc ~ vpd + I(vpd^2) + dbh + density + plot + year,
             data = data_fe)
...
*Response:* the vpd coefficient increased by a factor of 2 which suggests that something with the plot was confounding the VPD estimate.
...
*Response:* An El Nino or La Nina weather event
...
*Response:* Plot density, reef slope, any physical effects of a plot.
