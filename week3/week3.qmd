...
ggplot(weekly_la_phx, aes(x = week, y = mean_no2, color = city)) +
  ...
  # Add CA lockdown start dateline
...
No the trends violate the parallel trends assumption because they are drasticlly different in such a short area.
...
tx_nm_did <- lm(daily_max_1_hour_no2_concentration ~ treated * is_post, 
                data = filter(.data = did_data_nm_tx, 
                              year == 2020, 
                              month(date_obj) %in% 
                                c(2,3,4)))
```
...
Effect of being in the state of treatment (Texas?) on weekly average NO2 concentration. 
...
It appears to tell the whole story because the change could be due to something else and we wouldnt know without comaping to other years. 
...
Yeah close enough because the fluctuations are very similar despite not being exactly the same. But because of the small timeline its hard to assess a continuous change.
...
Beta 4, -3.5 which is not significant with a p-value of 0.22. The p value tells us that we cannot be sure if lockdown reduced NO2 concentration.
...
It should not ruin our difference-in-difference because it is affecting both groups equally but it does affect our estimate because it is introducing another variable.

Our counterfactual is the control variable.
...
Our DiD would get smaller because the effect of the treatment would be insignificant. The parallel trends would be more or less the same. 
