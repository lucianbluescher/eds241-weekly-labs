...
```
Make a new column of height in feet so I can figure out who everyone is!
```{r}
class_data <- class_data %>%
  mutate(height_ft_in = paste0(floor(height / 12), "'", height %% 12))
```
...
tab_source_note(
    "Source: Moland et al., 2013")#### Time to get REALLY fancy! 
...
table_w_plots <- lobsters |> 
  # Calculate total cpu for each year
  group_by(year) |> 
  summarize(
    total_cpue = sum(cpue, na.rm = TRUE),
...
library(cowsay)
library(beepr)
say("All done making some beautiful tables! :) ", "yoda"); beep(8)  
