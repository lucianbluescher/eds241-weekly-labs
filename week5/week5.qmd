...
```{r}
rdd_lm <- ggplot(data = data_binned, aes(x = avg_distance, y = avg_deforest, color = as.factor(protected))) +
...
0 is not protected on the left and represents distance away from the cutoff (park boundary), 1 is protected on the right is also distance into park from boundary. 
...
No discontinuity would be protected area effectiveness is non-existent. 
...
rdd_ols <- lm(
  deforest ~ # outcome
    ...
    protected * distance + # allows sloped to vary
    slope_cat + road_cat + water_cat + soil_cat,
  data = sim_data
)
...
-.1
protected areas decrease deforestation by 10%
0.002 represents the slope in protected areas difference to non-protected areas
**RESPONSE**:
...
global_samp <- sim_data %>%
     sample_n(size = nrow(sim_data) * .5) # <<< e.g., .5 for 50%
```
...
Assigning more weight to points closer to the cutoff reduces the decreasing effect of protected area.
...
Binned linear is well, linear and doesn't show us the curvature of the slope, local polynomial does.
